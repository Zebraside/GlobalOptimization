...
void StronginSolverMethod::performTest(IFunction& f, double a, double b, double epsilon, int n)
{
...
void StronginSolverMethod::evaluateSolution()
{
    ...
    minimumCharacteristicsValue=*std::min_element(characteristics.begin(), 
                                             characteristics.end());
    minimumCharacteristicsIdx=std::distance(characteristics.begin(),
...
double StronginSolverMethod::calculateLipschitzConstant()
{
    ...
    double maxValue=*std::max_element(value.begin(), value.end());
    if(maxValue==0)
    {
        return 1;
    }
